...
These files target KEMS 0.7.0-alpha1. Home Assistant may append `_2` if old entity-registry entries still use an ID. The diagnostic dashboard discovers KEMS entities dynamically and is not affected by suffixes.
...
lists every entity shipped by KEMS 0.7.0-alpha1, including Power Down planning and reward estimates. Use it for screenshots and pair
