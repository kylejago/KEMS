...
Rate and demand checks are corroborating evidence rather than independent permission: neither a cheap-looking rate nor a high site demand can authorise daytime battery import without the active Intelligent window and live Ohme charging confirmation. Current demand is used only when the configured corroborating source is an Octopus Energy demand sensor; its absence does not invent a negative reading.
