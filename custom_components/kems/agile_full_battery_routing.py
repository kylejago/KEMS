"""Canonical full-battery solar spill routing for Full KEMS Agile.

This Alpha8 module carries forward the proven Alpha7.48 current-routing
reconciliation without keeping that version-named patch in the executable
runtime chain. At authoritative 100% Agile SOC it blocks impossible charging,
preserves solar-to-home, and routes remaining PV to paid export within shared
inverter/export limits or explicitly curtails it.
...
def _full_battery_snapshot(self, state: dict[str, Any]) -> dict[str, Any]:
    ...
    snapshot = _original_snapshot(self, state)
...
def install_full_battery_routing() -> None:
    """Install the proven full-SOC current-routing reconciliation."""
    current_snapshot = alpha730._snapshot
    if getattr(current_snapshot, "_kems_full_battery_routing", False):
        return

    global _original_snapshot
    _original_snapshot = current_snapshot
    _full_battery_snapshot._kems_full_battery_routing = True
