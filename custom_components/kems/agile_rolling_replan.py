...
def _rolling_plan(
    ...
    *,
    ...
    remaining = max(desired - sum(item["allocation_kwh"] for item in candidates), 0.0)
...
def _publish_with_rolling(self, state: dict[str, Any]) -> None:
    ...
    if isinstance(now, datetime) and isinstance(config, SimulationConfig) and isinstance(
        tariff, TariffSettings
    ):
