...
from .no_export_cheap_policy import route_no_export_cheap
...
class SimulationEngine:
    ...
    def __init__(self) -> None:
        """Keep completed day-start replays cheap across coordinator updates."""
        self._day_start_cache: dict[
            tuple[datetime, SimulationConfig, int, datetime], float
        ] = {}
    ...
    def simulate_today(
        ...
        records_by_day = _records_by_day
        if records_by_day is None:
            records_by_day = {}
            for record in records:
                records_by_day.setdefault(record.timestamp.date(), []).append(record)
            for day_records in records_by_day.values():
                day_records.sort(key=lambda record: record.timestamp)
        today = records_by_day.get(now.date(), [])
        ...
        assert battery_kwh is not None
        ...
        replay_records = today
        if (
            _day_end_boundary is not None
            and _day_end_boundary.timestamp > today[-1].timestamp
        ):
            replay_records = [*today, _day_end_boundary]

        for index, (current, following) in enumerate(
            zip(replay_records, replay_records[1:], strict=False)
        ):
            ...
            if self._saving_session_active(current, config) and export_tariff_active:
                ...
                # This route models the agreed EV/grid-only policy. Normal
                # overnight demand is forecast AFTER the cheap period; a daytime
                # Intelligent slot protects the battery until the next cheap start.
                overnight = current.off_peak is True
                (
                    needed_home,
                    forecast_source,
                    _forecast_home,
                    ...
                    config,
                    forecast_energy_until_offpeak_kwh,
                    exclude_active_cheap=overnight,
                )
                target_stored = (
                    self._no_export_charge_target_stored_kwh(
                        needed_home, reserve_kwh, capacity, config
                    )
                    if forecast_source != "unavailable"
                    else None
                )
                flow = route_no_export_cheap(
                    whole_site_load_kw=load_kw,
                    ev_power_kw=current.ev_power_kw,
                    ev_charging=current.ev_charging,
                    ev_power_stale="ev_power_kw" in current.stale_fields,
                    solar_kw=solar_kw,
                    battery_stored_kwh=battery_kwh,
                    target_stored_kwh=target_stored,
                ...
                battery_kwh = flow.battery_stored_kwh
                stored_solar = (
                ...
                interval_import = flow.grid_import_kwh
                interval_export = 0.0
                interval_solar_to_home = flow.solar_to_home_kwh
                interval_solar_to_battery = stored_solar
                interval_grid_to_battery = stored_grid
                interval_curtailment = flow.solar_curtailed_kwh
            elif current.cheap_period_confirmed:
                # Paid-export Full KEMS: retain the original charge-to-full
                # and solar-export branch, independently of no-export routing.
                ...
                interval_export, interval_curtailment = self._limit_export(
                    solar_left_kwh, export_capacity
                )
    ...
    def _battery_energy_at_day_start(
        ...
        reserve_kwh: float,
        config: SimulationConfig,
        records_by_day: dict[date, list[Snapshot]],
    ) -> float:
        """Carry the previous simulated battery state across midnight."""
        initial = (
            capacity
            * min(
        ...
        initial = min(max(initial, reserve_kwh), capacity)
        previous_day = first_today.timestamp.date() - timedelta(days=1)
        previous_day_records = records_by_day.get(previous_day, [])
        if not previous_day_records:
            return initial

        latest_previous = previous_day_records[-1]
        if (
            first_today.timestamp - latest_previous.timestamp
        ...
        cache_key = (
            first_today.timestamp,
            config,
            len(previous_day_records),
            latest_previous.timestamp,
        )
        cached = self._day_start_cache.get(cache_key)
        ...
        boundary = (
            first_today if 0.0 < boundary_gap_hours <= MAX_INTERVAL_HOURS else None
        )
        previous_state = self.simulate_today(
            records,
            latest_previous.timestamp,
            config,
            current_snapshot=latest_previous,
            _day_end_boundary=boundary,
            _records_by_day=records_by_day,
        )
        if previous_state.simulated_battery_soc is None:
            return initial

        carried = (
            capacity
            * min(
                max(previous_state.simulated_battery_soc, 0.0),
                100.0,
            )
            / 100
        )
        carried = min(max(carried, reserve_kwh), capacity)
        if len(self._day_start_cache) >= 256:
            self._day_start_cache.clear()
        self._day_start_cache[cache_key] = carried
        return carried
    ...
    def _current_plan(
        ...
        if snapshot.cheap_period_confirmed:
            if no_export_mode:
                overnight = snapshot.off_peak is True
                (
                    required_home,
                    reserve_source,
                    forecast_home,
                ...
                flow = route_no_export_cheap(
                    whole_site_load_kw=load,
                    ev_power_kw=snapshot.ev_power_kw,
                    ev_charging=snapshot.ev_charging,
                    ev_power_stale="ev_power_kw" in snapshot.stale_fields,
                    solar_kw=solar,
                    battery_stored_kwh=battery_kwh,
                    target_stored_kwh=target_stored,
                    reserve_kwh=reserve_kwh,
                    ...
                    max_charge_kw=config.max_charge_kw,
                    max_discharge_kw=config.max_discharge_kw,
                    inverter_limit_kw=inverter_limit,
                    site_import_limit_kw=config.site_import_limit_kw,
                    charge_efficiency=config.charge_efficiency,
                    discharge_efficiency=config.discharge_efficiency,
                    hours=1.0,
                )
                site_headroom, site_exceeded = self._site_import_status(
                    flow.grid_import_kwh, config
                )
                return {
                    "house": round(load, 3),
                    "solar": round(solar, 3),
                    "grid_import": round(flow.grid_import_kwh, 3),
                    "grid_export": 0.0,
                    "battery": round(
                        flow.battery_to_home_kwh
                        - (
                    ...
                    "solar_to_battery": round(flow.solar_to_battery_input_kwh, 3),
                    "battery_to_home": round(flow.battery_to_home_kwh, 3),
                    "battery_export": 0.0,
                    "target_battery_export": 0.0,
                    "total_kh7_output": round(
                        flow.solar_to_home_kwh + flow.battery_to_home_kwh, 3
                    ),
                    "grid_bypass": round(
                        flow.grid_to_ev_kwh + flow.grid_to_home_kwh, 3
                    ),
                    "total_site_import": round(flow.grid_import_kwh, 3),
                    ...
                    "export_paused_for_home": False,
                    "overnight_charge_target_percent": (
                        round(100 * target_stored / capacity, 1)
