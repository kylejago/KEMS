...
disabled by default, so KEMS prefers that public state when present and otherwise
reads the integration's read-only coordinator result.  No Octopus entities,
settings, services or coordinator state are modified.
...
def _event_dict(value: Any, *, source: str, source_entity: str | None) -> dict[str, Any] | None:
...
def _public_power_up_events(hass: Any, account_hint: str | None) -> tuple[list[dict[str, Any]], bool]:
    """Read enabled BottlecapDave Power Up event entities without mutating them."""
    ...
    for state in all_states:
        ...
        if not lowered.startswith("event.octopus_energy_") or not lowered.endswith(_POWER_UP_SUFFIX):
            continue
    ...
    for raw in raw_events:
        event = _event_dict(raw, source="public_event_entity", source_entity=state.entity_id)
        if event is not None:
            output.append(event)
    return output, True


def _coordinator_power_up_events(hass: Any, account_hint: str | None) -> tuple[list[dict[str, Any]], bool, str | None]:
    """Read BottlecapDave 19.0.1+ coordinator data as a fail-safe fallback."""
    hass_data = getattr(hass, "data", {})
    domain_data = hass_data.get(_OCTOPUS_DOMAIN) if isinstance(hass_data, Mapping) else None
    if not isinstance(domain_data, Mapping):
        return [], False, None
    candidates = []
    for account_id, account_data in domain_data.items():
        if not isinstance(account_data, Mapping) or _COORDINATOR_KEY not in account_data:
            continue
    ...
    output = []
    for raw in raw_events:
        event = _event_dict(raw, source="octopus_coordinator", source_entity=None)
        if event is not None:
            output.append(event)
...
def _merge_consecutive(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for event in ordered:
        ...
        combined_minutes = (event["end"] - groups[-1][0]["start"]).total_seconds() / 60.0
        ...
        merged.append(
            {
                **first,
                "end": last["end"],
                "duration_minutes": (last["end"] - first["start"]).total_seconds() / 60.0,
...
def _select_candidate(events: list[dict[str, Any]], now: datetime) -> tuple[dict[str, Any] | None, str]:
...
def automatic_happy_hour_event(
    ...
    *,
    ...
    source_account = None
    if not events:
        coordinator_events, coordinator_supported, source_account = _coordinator_power_up_events(
            hass, account_hint
        )
    ...
    if candidate is not None:
        duration_hours = max((candidate["end"] - candidate["start"]).total_seconds() / 3600.0, 0.0)
        ...
        return {
            ...
            "event_ids": candidate.get("event_ids") or ([candidate.get("id")] if candidate.get("id") else []),
    ...
    manual.update(
        {
            ...
            "automatic_status": status if source_supported else "power_up_source_unavailable",
            "source_kind": source_kind,
            "source_account": source_account,
...
def install_automatic_happy_hour() -> None:
    ...
    def resolved_event(self) -> dict[str, Any]:
        ...
        entry = self._hass.config_entries.async_get_entry(str(entry_id)) if entry_id else None
    ...
    def context_with_source(self, state, *, now, config, tariff, power_down, safe_available_kwh=None):
