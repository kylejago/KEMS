"""Alpha7.44 same-window Agile totals and compact slot decision table.

The focused Agile page must compare like with like. Alpha7.42 exposed the right
live/simulated power streams, but its totals card reconstructed simulated house
energy from grid flows and the period aggregator omitted several solar/battery
flow fields. That made a large overnight grid-to-battery charge look like house
consumption and could show zero simulated solar even while the live digital twin
was actively routing solar.

Alpha7.44 keeps the existing optimiser untouched and fixes only reporting:

* simulated house energy is measured directly from the same retained demand
  intervals consumed by the Agile replay;
* period aggregation carries solar generation, solar-to-home and grid-to-battery
  evidence instead of dropping it;
* the Today table compares actual and Agile simulation over the same
  midnight-to-now window and uses the headline bill basis for the main cost;
* every Octopus Agile half-hour settlement slot is listed with the current KEMS
  decision, including unpublished-price holds, Power Down and Happy Hour states.
...
_ALPHA744_SENSOR_IDS = (_TODAY_SUMMARY, _SLOT_DECISIONS)
...
def _augment_aggregate(
    ...
    result = alpha744_original_aggregate(days, key, label)
...
def improve_alpha744_dashboard(content: str) -> str:
    """Replace the incorrect totals card and add the compact slot table."""
    start = content.find(_TODAY_CARD_START)
    if start < 0:
        raise ValueError("Alpha7.44 Today totals dashboard marker missing")
    plan = content.find(_PLAN_GRID_START, start)
    if plan < 0:
        raise ValueError("Alpha7.44 Plan now dashboard marker missing")
    return content[:start] + _REPLACEMENT_CARDS.rstrip() + "\n\n" + content[plan:]


def install_alpha744_dashboard_parity_patch() -> None:
    """Install same-window totals, period flow parity and slot decisions."""
    global alpha744_original_aggregate

    aggregate = agile._aggregate
    if not getattr(aggregate, "_kems_alpha744_dashboard_parity", False):
        alpha744_original_aggregate = aggregate
        _augment_aggregate._kems_alpha744_dashboard_parity = True
        agile._aggregate = _augment_aggregate

    agile_day = agile.AgileSmartExportManager._agile_day
    if not getattr(agile_day, "_kems_alpha744_dashboard_parity", False):
        original_agile_day = agile_day

        def agile_day_with_alpha744(self, records, *args, **kwargs):
            summary, plan = original_agile_day(self, records, *args, **kwargs)
            summary = dict(summary)
            summary["house_load_kwh"] = _house_energy(records)
            return summary, plan

        agile_day_with_alpha744._kems_alpha744_dashboard_parity = True
        agile.AgileSmartExportManager._agile_day = agile_day_with_alpha744

    publish = runtime.EfficientAgileSmartExportManager._publish
    if not getattr(publish, "_kems_alpha744_dashboard_parity", False):
        original_publish = publish

        def publish_with_alpha744(self, state: dict[str, Any]) -> None:
        ...
        publish_with_alpha744._kems_alpha744_dashboard_parity = True
        runtime.EfficientAgileSmartExportManager._publish = publish_with_alpha744

    shutdown = runtime.EfficientAgileSmartExportManager.async_shutdown
    if not getattr(shutdown, "_kems_alpha744_dashboard_parity", False):
        original_shutdown = shutdown

        async def shutdown_with_alpha744(self) -> None:
            await original_shutdown(self)
            for entity_id in _ALPHA744_SENSOR_IDS:
                self._hass.states.async_remove(entity_id)

        shutdown_with_alpha744._kems_alpha744_dashboard_parity = True
        runtime.EfficientAgileSmartExportManager.async_shutdown = shutdown_with_alpha744

    from . import dashboard as dashboard_module

    combined = dashboard_module._combined_master_dashboard_bytes
    if getattr(combined, "_kems_alpha744_dashboard_parity", False):
        return
    original_dashboard = combined

    def combined_alpha744_dashboard() -> bytes:
        content = original_dashboard().decode("utf-8")
        return improve_alpha744_dashboard(content).encode("utf-8")

    combined_alpha744_dashboard._kems_alpha744_dashboard_parity = True
    dashboard_module._combined_master_dashboard_bytes = combined_alpha744_dashboard
