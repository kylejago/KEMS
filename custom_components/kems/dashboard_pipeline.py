...
def _finalise_dashboard_bytes(payload: bytes) -> bytes:
    ...
    new_saving_line = (
        "          **KEMS saving today:** {{ ('£%.2f' | format((saving | float) / 100)) if saving is not none else '—' }}\n\n"
        "          Total energy cost includes electricity standing charge, export income, supplier/account credits and gas.\n"
    )
