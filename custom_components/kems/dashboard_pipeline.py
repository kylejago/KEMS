...
def _plan_table_card(
    *,
    ...
    return (
        ...
        "half-hour (or the remaining part of the active half-hour). Historical "
        "runtime/data gaps show **NO DATA** rather than zero activity.\n\n"
        ...
        f"{content}{{%- set gap = p.get('flow_basis') == "
        "'settled/replayed KEMS slot' and "
        "p.get('actions') == ['future slot'] %}\n"
        ...
        "else '—' }} | "
        "{{ '—' if gap else (('%.1f%%' | format(soc | float(0))) "
        "if soc is not none else '—') }} | "
        "{{ '**NO DATA** · —' if gap else "
        "('**' ~ ga ~ '** · ' ~ (('%.2f kWh' | format(gk | float(0))) "
        "if gk is not none else '—')) }} | "
        "{{ '**NO DATA** · —' if gap else "
        "('**' ~ sa ~ '** · ' ~ (('%.2f kWh' | format(sk | float(0))) "
        "if sk is not none else '—')) }} | "
        "{{ '**NO DATA** · —' if gap else "
        "('**' ~ ba ~ '** · ' ~ (('%.2f kWh' | format(bk | float(0))) "
