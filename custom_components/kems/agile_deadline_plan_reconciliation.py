"""Canonical deadline/price-plan reconciliation for Agile Smart Export.

This Alpha8 module carries forward the proven Alpha7.49 and Alpha7.51 behaviour
without keeping those version-named patch modules in the executable runtime
chain. It preserves the solar-aware deadline coverage check, equal-energy plan
rebalance, and maximum-discharge plan reconciliation.
...
def _dispatch_with_plan_coverage(
    self,
    state: dict[str, Any],
    plan: dict[str, Any],
    *,
    ...
    targets = _original_plan_coverage_dispatch(
        self,
        state,
        plan,
        now=now,
...
def _dispatch_with_maximum_discharge_reconcile(
    self,
    state: dict[str, Any],
    plan: dict[str, Any],
    *,
    ...
    targets = _original_maximum_discharge_dispatch(
        self,
        state,
        plan,
        now=now,
...
def install_deadline_plan_coverage() -> None:
    """Install the proven economic/deadline coverage reconciliation."""
    dispatch = alpha717._dispatch_targets
    if getattr(dispatch, "_kems_deadline_plan_coverage", False):
        return

    global _original_plan_coverage_dispatch
    _original_plan_coverage_dispatch = dispatch
    _dispatch_with_plan_coverage._kems_deadline_plan_coverage = True
    alpha717._dispatch_targets = _dispatch_with_plan_coverage
