...
def _new_tracker(
    ...
    *,
    ...
    parent_solar = max(_number(today.get("solar_export_kwh")) or 0.0, 0.0) if today else 0.0
    parent_income = max(_number(today.get("export_income_pence")) or 0.0, 0.0) if today else 0.0
    ...
    if isinstance(previous, dict):
        ...
        if increment is None:
            ...
            if previous_ts is not None and current_ts is not None and current_ts > previous_ts:
                updated["skipped_gap_count"] = int(updated.get("skipped_gap_count", 0)) + 1
        else:
...
def _apply_elapsed_solar_accounting(
    ...
    *,
    ...
    today.update(
        {
            ...
            "gain_vs_fixed_12p_same_dispatch_pence": round(export_income - fixed_income, 2),
    ...
    checks["grid_export_balance"] = abs(
        grid_export - (solar_export + settled_battery_export)
    ) <= 0.002
    ...
    diagnostic.update(
        {
            ...
            "elapsed_live_solar_tracking_started_at": tracker.get("tracking_started_at"),
...
def _rebase_display_soc(
    ...
    *,
    ...
    slots = [slot for slot in state.get("today_slots", []) or [] if isinstance(slot, dict)]
    slots.sort(key=lambda slot: _dt(slot.get("valid_from")) or datetime.max.replace(tzinfo=UTC))
    active_index = next(
        (
            index
    ...
    battery_kwh = _clamp(battery_kwh + _battery_delta_kwh(active, config), 0.0, capacity)
    corrected_active_soc = 100.0 * battery_kwh / capacity
    active["flow_estimated_soc_percent"] = round(corrected_active_soc, 1)
    active["flow_soc_basis"] = "canonical current SOC + remaining displayed battery flow"
    ...
    for slot in slots[active_index + 1 :]:
        ...
        slot["flow_soc_basis"] = "rebased from canonical active SOC through displayed flow deltas"
